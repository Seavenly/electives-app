...
const command = new Promise((resolve, reject) => {
  ...
  if (args[0] === Flags.Reset) {
    p = tools
      .resetElectives()
      .then(() => resolve('Electives successfully reset.'));
  } else if (args[0] === Flags.Setup) {
    p = tools
      .fromCsv()
      .then(() => resolve('Students and lists successfully created.'));
  } else if (args[0] === Flags.Run) {
    p = tools.assignElectives().then(logger => {
      let log = '';
      ...
      logger.log.forEach((lineArr, index) => {
        ...
        if (lineArr[0] === 'HEAD')
          line = chalk.bold(`\n\n=== ${lineArr[1]} ===\n\n`);
        else if (lineArr[0] === 'TEXT')
          line = `${padstr(index + 1, 4)}  ${padstr(lineArr[0], 8)}   ${
            lineArr[1]
          }\n`;
        else if (lineArr[0] === 'INFO')
          line = `${padstr(index + 1, 4)}  ${chalk.bold.cyan(
            padstr(lineArr[0], 8),
          )}   ${chalk.cyan(lineArr[1])}\n`;
        else if (lineArr[0] === 'SUCCESS')
          line = `${padstr(index + 1, 4)}  ${chalk.bold.green(
            padstr(lineArr[0], 8),
          )}   ${chalk.green(lineArr[1])}\n`;
        else if (lineArr[0] === 'ERROR')
